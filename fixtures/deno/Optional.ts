...
import { constant, flow, Predicate, Refinement } from 'https://raw.githubusercontent.com/michaelhirn/fp-ts/master/lib/function.ts'
...
const imap_: Invariant2<URI>['imap'] = (ea, ab, ba) => optional(flow(ea.getOption, O.map(ab)) as (s: never) => O.Option<never>, flow(ba, ea.set))
